...
async function main() {
  ...
  try {
    ...
    `Requirements:`,
    `- Read and strictly follow ALL standards in DAILY_SPEC.md (CRITICAL: follow "Drag & Drop Safety" for interactive apps)`,
    `- Language: Use ${LANG} for ALL UI text, labels, and content.`,
    `- Tech stack: ${chosenUI} + ${chosenStyling}. For complex animations or drag-and-drop, PREFER using 'framer-motion' (it is already common, no extra setup needed).`,
    ...
    `\nIMPORTANT: You are in an automated pipeline. DO NOT ASK QUESTIONS. DO NOT EXPLAIN. JUST OUTPUT THE CODE.`,
    `CRITICAL: Keep the solution focused. Limit code changes to ~3-5 essential files.`,
    `CRITICAL: MUST include mock data to make the app useful immediately.`,
